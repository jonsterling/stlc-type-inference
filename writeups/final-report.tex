...
example, we denoted the type $\TyUnit \to (\TyUnit \to \TyUnit)$ as
\verb!> 1 (> 1 1)!. We made this design choice because the
sequence-to-sequence model processes input sequences sequentially, and
we hypothesised prefix notation might indicate what kind of data to
expect next in the sequence.
...
type inference algorithm infers the type $\TyArr\tau\tau$ for the term
$\Fn{x}{x}$ for some type variable $\tau$, we assigned the term the type
$\TyArr\TyUnit\TyUnit$.
