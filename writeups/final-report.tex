...
embedding $\st{t}$ at time step $t$, then
...
Given a source expression $S$ and target type $\tau$, these models
are used to model the probability $P(\tau \mid S)$. The basic idea of the
...
and decodes it to generate a target type $\tau$.
