...
expression $e$ as a sequence of characters ${(\st{t})}_{t \geq 1}$,
with character $\st{t}$ at time step $t$, then an RNN simply
...
\[
  \Define{
    ...
    \begin{cases}
      \tanh(W_{sh}\st{t} + W_{hh}\Ht{t-1} + \mathbf{b}_h) & \OBox{\IsGEQ{t}{q}}\\
...
backward over the input sequence of characters
${(\st{t})}_{t \geq 1}$.
...
\begin{align*}
  ...
  \ADefine{
    ...
    \begin{cases}
      \RNNb{\st{t}}{\Htb{t+1}}& \IsLEQ{t}{\Abs{S}}\\
      \mathbf{0} & \text{otherwise}
    \end{cases}
  }
\end{align*}
...
concatenating the vectors $\Htf{\Abs{S}}$ and $\Htb{1}$. Hence, the
...
\[
  \Define{H}{
    \text{concat\_col}\left(\Ht{1}, \Ht{2}, \ldots, \Ht{\Abs{S}}\right)
  }
\]

That describes the encoder model. We use an attention decoder to
generate the type from this bidirectional representation. This decoder
calculates a vector $\Al{t}$ that can be used to combine
together the columns of $H$ into a vector
$\Define{\mathbf{c}_t}{H \Al{t}}$. The attention vector
  basically denotes the focussing on a particular source word at a
